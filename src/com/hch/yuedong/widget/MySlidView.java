...
public class MySlidView extends RelativeLayout {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			if (mIsBeingDragged) {
				...
				if ((mSlidView.getScrollX() <= 0)
						&& (mSlidView.getScrollX() >= -(MyApplication.getInstance().width - MyApplication.getInstance().height * 50))) {
	...
	public void setSlidView(View view) {
		LayoutParams bgShadeParams = new LayoutParams(
				MyApplication.getInstance().width, MyApplication.getInstance().height);
		bgShadeParams.addRule(RelativeLayout.CENTER_IN_PARENT);
		bgShade.setLayoutParams(bgShadeParams);

		LayoutParams bgParams = new LayoutParams(MyApplication.getInstance().width,
				MyApplication.getInstance().height - 48);
	...
	private void showVelocityMenuView(int velocity) {
		if (velocity > 0) {
			if (velocity > SNAP_VELOCITY) {
				...
				if (mSlidView.getScrollX() > -MyApplication.getInstance().width / 2) {
		...
		if (velocity == 0) {
			if (mSlidView.getScrollX() > -(MyApplication.getInstance().width - MyApplication.getInstance().density * 50) / 2) {
				smoothScrollTo(-mSlidView.getScrollX());
			} else {
				smoothScrollTo(-(menuWidth + mSlidView.getScrollX()));
				setMenuState();
